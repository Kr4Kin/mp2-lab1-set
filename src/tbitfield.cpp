...
{
    pMem[n / (sizeof(elType) * 8)] = pMem[n / (sizeof(elType) * 8)] | getMask(getIndex(n));
}
...
{
    elType a = pMem[n / (sizeof(elType) * 8)];
    elType b = getMask(getIndex(n));
    if (a & b)
        return true;
    return false;
}
...
TBitField TBitField::operator~() // отрицание
{
    ...
    elType z = 0;
    for (int i = 0; i < bitLen % (sizeof(elType) * 8); i++) {
        z += (elType)1 << i;
    }
